  ...
  for (groups in c("CTR_CRC")) {
    for (data_type in c("Raw", "Raw_log", "Batch")) {
      ...
      meta.all <- subset(meta.all, Study %in% c(
        ...
        'GE-CRC', 'CHN_HK-CRC', 'CHN_SH-CRC', 'IND-CRC', 'US-CRC'
      ))
      ...
      # Save results to CSV
      df_adonis_result <- as.data.frame(matrix(nrow = 2, ncol = 5, 0))
      rownames(df_adonis_result) <- c("Study", "Group")
...
for (class_dir in c("class", "genus", "order", "family", "t_sgb", "species", "ko_gene", "uniref_family")) {
  for (groups in c("CTR_CRC")) {
    for (data_type in c("Raw", "Raw_log", "Batch")) {
      ...
      # Load feature data based on data type
      feature_file <- if (data_type == "Raw" || data_type == "Raw_log") {
        file.path(data_script_dir, class_dir, paste0("feature_rare_", groups, ".csv"))
      } else {
        file.path(data_script_dir, class_dir, "CTR_CRC_adj_batch.csv")
      }
      feat.all <- read.csv(feature_file, sep = ifelse(data_type == "Batch", ",", "\t"), stringsAsFactors = FALSE, header = TRUE)
      ...
      feature <- read.csv(
        file = file.path(feature_script_dir, class_dir, data_type, groups, "adj_p_CTR_CRC.csv"),
        sep = ',', row.names = 1
      )
      filtered_features <- feature[feature[["maaslin2"]] <= significance_threshold, ]
      
      # Subset features and metadata
      selected_row_names <- rownames(filtered_features)
